...
describe('cloudworker-e2e', async () => {
  ...
  test('Request cf botScore', async (cb) => {
    const script = `
      addEventListener('fetch', event => {
        const req = event.request
        const cloned = req.clone()
        const resp = new Response('', {
        headers: {
          'x-botScore': cloned.cf.botManagement.score,
          }
        })
        event.respondWith(resp)
      })
    `
    const server = new Cloudworker(script).listen(8080)
    const res = await axios.get('http://localhost:8080', Object.assign({}, defaultAxiosOpts, {
      headers: {
        'x-cloudworker-botscore': 123,
      },
    }))
    expect(res.headers['x-botscore']).toEqual('123')
    await server.close()
    cb()
  })
